...
class ProfileForm(ModelForm):
    error_messages = {
        'required': (_("This field is mandatory.")),
    ...
    esewa_id = forms.CharField(error_messages=error_messages,
                               ...
                               widget=forms.NumberInput(attrs={
                                                        'class': 'NumberField', 'type': 'number', 'placeholder': 'Enter your Esewa_Id'}),
...
class GraphicsForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['publisher'].widget.attrs.update(  # remove the select option
            {'class': 'd-none'})
        # remove the label element for parent(text wont appear)
        self.fields['publisher'].label = ''
        # to remove input required atttribute for parent so its gonna be optional
