...
class Profile(models.Model):  
	...
	esewa_id=models.PositiveIntegerField(null=True
								 ,help_text='This information will be used only for payment purpose.',blank=True
								,validators=[MinValueValidator(9000000000),MaxValueValidator(9999999999)]
								 )
...
class DiscountCoupon(models.Model):
	 discountcode=models.CharField(null=True
                                ,help_text='For example:- sam3432'
                                ,max_length=100)
	 discount =models.PositiveIntegerField(null=True
								  ,verbose_name="Discount in %"      
								 ,help_text='You can set discount from 1 to 90 %. Set the disount wisely.Note,DI do not accept decimal value.'
...
class Order(models.Model):
	...
	discountcoupon=models.ForeignKey(DiscountCoupon,on_delete=models.SET_NULL,null=True,blank=True)
...
#totaamountl of all products
	...
	def grand_totalamount(self):
		if self.discountcoupon:
				stotal=float(self.get_sfxproduct_total)+float(self.get_vfxproduct_total)+float(self.get_graphicsproduct_total)			   
				total=stotal-self.discountcoupon.discount/100*stotal
		else:            
				total=float(self.get_sfxproduct_total)+float(self.get_vfxproduct_total)+float(self.get_graphicsproduct_total)	
		return total
...
#totalquantity of all products
	...
	def grand_totalquantity(self):
		total=self.get_sfxproduct_items+self.get_vfxproduct_items+self.get_graphicsproduct_items	
		return total
