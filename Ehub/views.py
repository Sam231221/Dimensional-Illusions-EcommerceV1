...
def ContactApi(request):
	if request.method=="POST":
		...
		if len(name)<=2 and len(title)<=4 and len(content)<=9:
			...
			instantcontact=Email(name=name, email=email, title=title, content=content)
			instantcontact.save()
...
def addcouponcode(request):
	if request.user.is_authenticated: 
		...
		if request.method=="POST":
				...
				getprofile = request.user.profile
				order, created = Order.objects.get_or_create(profile=getprofile, complete=False)  #we are setting the customer value on authebnticating the user
...
class KhaltiVerifyView(View):
	...
	def get(self, request, *args, **kwargs):
		...
		purchase= PurchasedProduct.objects.get(profile=getprofile, complete=False)    
  
		orderpaidvfxproduct = OrderPaidVfxProduct.objects.filter(profile=getprofile,complete=False)
		orderpaidsfxproduct = OrderPaidSfxProduct.objects.filter(profile=getprofile,complete=False)
...
def ProcessOrder(request):
	...
	if request.user.is_authenticated:
		...
		orderpaidvfxproduct = OrderPaidVfxProduct.objects.filter(profile=getprofile,complete=False)
		orderpaidsfxproduct = OrderPaidSfxProduct.objects.filter(profile=getprofile,complete=False)
		orderpaidgraphicsproduct = OrderPaidGraphicsProduct.objects.filter(profile=getprofile,complete=False)
	...
	if request.method == 'POST':
		...
		try:
			...
			if password1 == password2:
				...
				profile_obj =Profile.objects.create(username = user_obj ,email=email ,auth_token = auth_token)
				profile_obj.save()
...
def verify(request , auth_token):
	try:
		profile_obj = Profile.objects.filter(auth_token = auth_token).first() #checking if the profile exist through auth_token
